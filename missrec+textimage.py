...
class MISSRec(Transformer):
    def __init__(self, config, dataset):
        ...
        # --- 创新点初始化：User-aware Adaptive Fusion ---
        if 'text' in self.modal_type and 'img' in self.modal_type:
            # 门控线性层：基于用户序列表示决定模态权重
            self.beta_linear = nn.Linear(config['hidden_size'], config['hidden_size'])
            # 显式 Cross-Attention：让用户表示与多模态序列进行交互
    ...
    def get_decoder_attention_mask(self, enc_input_seq, item_modal_empty_mask, is_casual=True):
        ...
        if is_casual:
            attn_mask = torch.triu(torch.full((seq_len, seq_len), float('-inf'), device=enc_input_seq.device), diagonal=1)
            if self.seq_mm_fusion != 'add':
    ...
    def forward(self, item_seq, item_emb, item_modal_empty_mask, item_seq_len, interest_seq=None, interest_emb=None, interest_seq_len=None):
        ...
        if self.train_stage == 'transductive_ft':
            if self.id_type != 'none':
                ...
                if self.seq_mm_fusion != 'add':
                    item_id_embeddings = item_id_embeddings.unsqueeze(1)
        ...
        # Contextual Encoder (Interest Extractor)
        memory = self.trm_model.encoder(src=enc_input_emb, mask=src_attn_mask, src_key_padding_mask=src_key_padding_mask)

        # Diversity Regularization
        src_key_token_weight = (~src_key_padding_mask).unsqueeze(-1).float().mean(1, keepdim=True)
        pooled_memory = (memory * src_key_token_weight).sum(1)
        interest_orthogonal_regularization = (pooled_memory * pooled_memory).sum() / pooled_memory.shape[1]
        ...
        # --- 核心创新逻辑：User-aware Adaptive Multimodal Fusion ---
        if 'text' in self.modal_type and 'img' in self.modal_type:
            # 1. 计算用户感知门控信号 beta
            beta = torch.sigmoid(self.beta_linear(output)) # [B, D]
            
            # 2. 显式 Cross-Attention 交互 
            # query: 当前用户状态, key/value: 包含模态信息的 decoder 输入序列
            mm_interact, _ = self.cross_attn(
                query=output.unsqueeze(1),
                key=dec_input_emb,
                value=dec_input_emb,
                key_padding_mask=tgt_key_padding_mask # 确保关注的是有效项
            )
            mm_interact = mm_interact.squeeze(1) # [B, D]
            
            # 3. 自适应融合
            output = output + beta * mm_interact
        # ---------------------------------------------------------

        return output, interest_orthogonal_regularization.mean()
    ...
    def _compute_seq_embeddings_pretrain(self, item_seq, item_seq_len, text_emb, img_emb, text_emb_empty_mask=None, img_emb_empty_mask=None, unique_interest_seq=None, unique_interest_emb_list=None, unique_interest_len=None):
    ...
    def _compute_seq_embeddings(self, item_seq, item_seq_len):
        ...
        unique_interest_emb_list = self.interest_embeddings[unique_interest_seq]
    ...
    def _compute_test_item_embeddings(self):
        test_item_emb = 0
        if 'text' in self.modal_type:
            test_item_emb = test_item_emb + self.text_adaptor(self.plm_embedding.weight)
        if 'img' in self.modal_type:
            test_item_emb = test_item_emb + self.img_adaptor(self.img_embedding.weight)
    ...
    def _compute_dynamic_fused_logits(self, seq_output, text_emb, img_emb):
        ...
        if self.item_mm_fusion in ['dynamic_shared', 'dynamic_instance']:
            agg_logits = (modality_logits * F.softmax(modality_logits * self.fusion_factor.unsqueeze(-1), dim=-1)).sum(dim=-1)
        else:
